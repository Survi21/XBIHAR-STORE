...
// // Assuming UserDropdown is imported or defined in your code
// // import UserDropdown from "./UserDropdown";
...
//   // 2. FIXED OUTSIDE CLICK LISTENER
//   useEffect(() => {
...
//             {/* 🚨 FIXED USER ICON & DROPDOWN WRAPPER */}
//             <div className="relative" ref={dropdownRef}>
//               {user ? (
//                 <button
//                   type="button"
//                   onClick={() => setShowDropdown((prev) => !prev)}
//                   className="cursor-pointer select-none focus:outline-none block"
//                 >
//                   <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-white text-black flex items-center justify-center text-xs md:text-sm font-bold shadow-md hover:bg-zinc-200 transition">
...
//               {/* DROPDOWN MENU */}
//               {showDropdown && user && (
//                 <div className="absolute right-0 top-11 z-50 min-w-[170px] bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden py-1">
//                   {/* Option 1: Profile */}
//                   <Link
//                     href="/profile"
//                     onClick={() => setShowDropdown(false)}
//                     className="block px-4 py-2.5 text-xs font-mono text-zinc-300 hover:bg-zinc-900 hover:text-white transition border-b border-zinc-900"
//                   >
//                     👤 MY PROFILE
//                   </Link>
...
//                     }}
//                     className="w-full text-left px-4 py-2.5 text-xs font-mono text-red-500 hover:bg-red-500/10 transition"
//                   >
//                     🚪 LOGOUT
//                   </button>
//                 </div>
...
//             <Link href="/product" onClick={() => setShowMenu(false)} className="hover:text-white transition">PRODUCT</Link>
//             <Link href="/wishlist" onClick={() => setShowMenu(false)} className="xs:hidden text-zinc-400 hover:text-white transition">WISHLIST</Link> 
//             <Link href="/orders" onClick={() => setShowMenu(false)} className="hover:text-white transition">MY ORDERS</Link>
//             <Link href="/profile" onClick={() => setShowMenu(false)} className="hover:text-white transition">PROFILE</Link>
//           </nav>
//         </div>
//       )}
//     </>
...
export default function Navbar() {
  ...
  // OUTSIDE CLICK LISTENER — dropdown close karne ke liye
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        dropdownRef.current &&
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full z-50 border-b border-zinc-800 bg-black/70 backdrop-blur">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between relative w-full">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            {/* USER ICON + DROPDOWN — sirf ISI pe click karne se khulega */}
            <div className="relative" ref={dropdownRef}>
              {user ? (
                <button
                  type="button"
              ...
              {/* DROPDOWN MENU — bas yahin dikhega, kahin aur nahi */}
              {showDropdown && user && (
                <div className="absolute right-0 top-11 z-50 min-w-[170px] bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden py-1">
                  <Link
                    href="/account"
      ...
      {/* MOBILE MENU (hamburger ☰) — sirf navigation links, Profile/Orders yahan NAHI */}
      {showMenu && (
        <div className="fixed inset-0 bg-black z-[999] overflow-y-auto">
          <div className="flex justify-between items-center p-6 border-b border-zinc-800">
            <h2 className="text-xl font-bold font-orbitron text-white">XBIHAR</h2>
